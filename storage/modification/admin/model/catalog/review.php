...
class ModelCatalogReview extends Model {
	public function addReview($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['author']) . "', product_id = '" . (isset($data['product_id']) ? (int)$data['product_id'] : '') . "', information_id = '" . (isset($data['information_id']) ? (int)$data['information_id'] : '') . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', reply = '" . $this->db->escape(strip_tags($data['reply'])) . "', rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_added = '" . $this->db->escape($data['date_added']) . "'");
	...
	public function editReview($review_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['author']) . "', product_id = '" . (isset($data['product_id']) ? (int)$data['product_id'] : '') . "', information_id = '" . (isset($data['information_id']) ? (int)$data['information_id'] : '') . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', reply = '" . $this->db->escape(strip_tags($data['reply'])) . "', rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_added = '" . $this->db->escape($data['date_added']) . "', date_modified = NOW() WHERE review_id = '" . (int)$review_id . "'");
	...
	public function getReview($review_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT pd.name FROM " . DB_PREFIX . "product_description pd WHERE pd.product_id = r.product_id AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS product, (SELECT id.title FROM " . DB_PREFIX . "information_description id WHERE id.information_id = r.information_id AND id.language_id = '" . (int)$this->config->get('config_language_id') . "') AS information FROM " . DB_PREFIX . "review r WHERE r.review_id = '" . (int)$review_id . "'");
